...
class Tensor:
    ...
    def __pow__(self, other):
        ...
        other = Tensor(other)
        out = Tensor(self.data ** other.data, (self,), op=self.__pow__)
        
        def grad_fn(gradient):
            self.grad += gradient * (other.data * (self.data ** (other.data-1)))
            #other.grad += gradient * (out.data * np.log(self.data + 1e-10))
        out.grad_fn = grad_fn
