...
class Tensor:
    def __init__(self, data, prev=(), op=lambda x: None, name=None, *args, **kwargs):
        ...
        if self.data.ndim < 2 or self.data.ndim > 4:
            raise ValueError(f'dim of data must be between 2-4 but is {self.data.ndim}')
    ...
    @staticmethod
    def var(a, m, axis=None, keepdims=False):
    ...
    def sum(self, axis=None, keepdims=False):
        ...
        def grad_fn(gradient):
            self.grad += np.broadcast_to(
                gradient if keepdims else np.expand_dims(gradient,axis=axis),input_shape
                )
        out.grad_fn = grad_fn
    ...
    def flatten(self):
        ...
        out = Tensor(self.data.flatten(), (self,), op=self.flatten)
...
class BatchNorm1D(Module):
    ...
    def __call__(self, x, training=True):
        ...
        if training:
            ...
            v = Tensor.var(x, m, axis=0, keepdims=True) + 1e-5
...
class LayerNorm(Module):
    ...
    def __call__(self, x):
        ...
        m = Tensor.mean(x, axis=self.normalized_shape, keepdims=True)
        v = Tensor.var(x, m, axis=self.normalized_shape, keepdims=True) + 1e-5
...
#TODO: make Conv2d more flexivle (stride,pad,...) and maybe faster
class Conv2d(Module):
